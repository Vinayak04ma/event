...
const C = () => {
  ...
  return (
   <div className="bg-[#04152d]">
      <div  >
        <h1 className=" font-bold text-[30px] text-[#e5e5e5] ml-6 mt-[50px]">Popular Events in INDIA</h1>
        <div className=" flex gap-6 ml-8 mt-[20px] text-[#e5e5e5] font-semibold text-[24px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {category.map((event, index) => (
            ...
            transition={{
             ...
             key={index} className="bg-[#579dffbf] cursor-pointer rounded-lg shadow-md overflow-hidden hover:border-red-500 transition-all">
              <img src={event.image_url} alt={event.event_name} className="w-full h-48 object-cover image-hover-effect" />
              <div className="p-4">
                <h3 className="text-xl font-semibold text-[#e5e5e5]">{event.event_name}</h3>
                <p className="text-[#e5e5e5]">Location: {event.location}</p>
                <p className="text-[#e5e5e5]">Fee: ₹{event.fee}</p>
              </div>
            </motion.div>
          ))}
        </div>
